...
def make_time_clip(time, tfont, tsize): 
    hhmmss = list(prnt_time_left(time)) 
    time_img = td.make_countdown_img(tfont, tsize, hhmmss) 
    return ImageClip(time_img) 
    
font_size_list = list(td.find_metrics(font_name, FONTSIZE)) 
...
while (not flag):
    ...
    for i in range(60):
        if(((n * 60) + i) > ttl_time): 
            break
        time = list(prnt_time_left((60 * n) + i))
        img = td.make_countdown_img(font_name, font_size_list, time) 
        print(img) 
        clip = ImageClip(img).set_position('center')  
        print(clip.size) 
        clip = clip.set_start(i)
        clip = clip.set_end(i + 1)
        clip_queue.append(clip)
    subfile = CompositeVideoClip(clip_queue, bg_color = (0, 0, 0)) 
    print(subfile.size)
    subfile.write_videofile("countdown_subfile{number}.mp4".format(number = n), fps = 24, threads = 1)
    file_queue.append(VideoFileClip("countdown_subfile{number}.mp4".format(number = n)).set_start((n, 0)).set_position('center'))
    clip_queue = []
    subfile = None
    n += 1
    if(n * 60 > ttl_time):
...
#for n in file_queue: 
#    n.set_position('center') 
#    print("offset " + str(n.pos))
#
...
end_clip = ImageClip(img).set_pos('center')
#print(end_clip.size)
end_clip = end_clip.set_start(ttl_time)
end_clip = end_clip.set_duration(60)
file_queue.append(end_clip)
